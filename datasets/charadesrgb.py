...
class Charades(data.Dataset):
    ...
    #     """
    #     vid, target, fm_NO, total_num = self.data[index]
    #     path = '{}/{}-{:06d}.jpg'.format(os.path.join(self.root, vid), vid, int(fm_NO))
    #     target = torch.from_numpy(target[:, 0].astype(np.float32))
    #     img = default_loader(path)
    #     raw_test = torch.FloatTensor(cv2.resize(np.array(img), (224, 224), interpolation=cv2.INTER_LINEAR))
    #     meta = {}
    #     meta['id'] = vid
    #     meta['time'] = int(fm_NO)
    #
    #     Org_W = img.size[0]
    #     Org_H = img.size[1]
    #     # Auxili_info = [index, int(fm_NO)]
    #     Auxili_info = [self.vdlist.index(vid), int(fm_NO)]
    #     Auxili_info.insert(1, Org_H)
    #     Auxili_info.insert(2, Org_W)
    #
    #     if self.transform is not None:
    ...
    #         target = self.target_transform(target)
    #
    #     return img, target, meta, vid, torch.FloatTensor(np.array(Auxili_info)).contiguous(), raw_test
